...
import { UserPlus, Loader2, Trash2, ImageUp, Edit2 } from "lucide-vue-next";
...
const addUser = async () => {
  if (!newUser.value.dni || !newUser.value.apellidos || !newUser.value.nombre)
    return;
  ...
  try {
    ...
    logInfo(
      `Usuario añadido: ${newUser.value.nombre} ${newUser.value.apellidos}`
    );
    newUser.value = {
      dni: "",
      apellidos: "",
...
const deleteUser = async (index) => {
  ...
  try {
    ...
    logInfo(
      `Usuario eliminado: ${deletedUser.nombre} ${deletedUser.apellidos}`
    );
  } catch (error) {
    ...
    <div class="flex flex-col gap-4 mb-6">
      <div class="flex justify-between items-center">
        ...
        <div class="h-full">
          ...
          <div class="w-40">
            ...
            <label
              for="imagen"
              class="cursor-pointer bg-white py-2 px-3 border-2 border-gray-300 rounded-md shadow-sm text-sm leading-4 font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 h-12 flex items-center justify-center w-full"
            >
              <ImageUp class="h-5 w-5 mr-2" />

              <span class="truncate">{{
                newUser.imagen ? newUser.imagen.name : "Imagen"
              }}</span>
            </label>
          </div>
        </div>
      ...
      <div class="flex justify-end">
        <button
          @click="addUser"
          :disabled="
            isLoading || !newUser.dni || !newUser.apellidos || !newUser.nombre
          "
          class="w-40 cursor-pointer inline-flex justify-center items-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 h-12"
        >
          <Loader2
            v-if="isLoading"
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(user, index) in users" :key="user.dni">
              ...
              <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-500">
                <img
                  v-if="user.imagen"
                  :src="URL.createObjectURL(user.imagen)"
                  alt="User Image"
