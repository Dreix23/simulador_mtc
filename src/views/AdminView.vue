  ...
  <div class="flex h-screen bg-gray-100">
    ...
    <aside :class="[
      ...
      <div class="p-4 flex justify-between items-center">
        <h2 :class="['font-bold text-xl', isSidebarOpen ? '' : 'hidden']">Admin Panel</h2>
        <button @click="toggleSidebar" class="text-gray-500 hover:text-gray-700">
          <Menu />
        </button>
      </div>
      <nav class="mt-8">
        <a @click="setActiveSection('users')" :class="[
        ...
        <a @click="setActiveSection('questionnaires')" :class="[
          'flex items-center p-4 text-gray-700 hover:bg-red-100 cursor-pointer',
          activeSection === 'questionnaires' ? 'bg-red-100' : ''
        ]">
          <FileQuestion :size="24" />
          <span :class="['ml-4', isSidebarOpen ? '' : 'hidden']">Agregar Cuestionario</span>
        </a>
      </nav>
    </aside>
